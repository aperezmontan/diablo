    ...
    context 'when requesting an HTML response' do
      ...
      context 'with bad parameters' do
        it 'fails' do
          ...
          expect(response).to have_http_status(200)
          expect(response.content_type).to eq('text/html')
          expect(response.body).to include('Teams [[&quot;Arizona Cardinals&quot;, &quot;Washington Redskins&quot;]] are playing each other')
        end
      end
    end
    ...
    context 'when requesting a JSON response' do
      ...
      context 'with bad parameters' do
        it 'fails' do
          ...
          expect(response).to have_http_status(422)
          expect(response.content_type).to eq('application/json')
          expect(JSON.parse(response.body)).to eq('teams' => ['[["Arizona Cardinals", "Washington Redskins"]] are playing each other'])
        end
      end
    end
  end
